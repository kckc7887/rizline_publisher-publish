...
首次导入需要读取完整上游资源，耗时受官方服务器网络影响；首次 Ubuntu 实测约 44 分钟。构建任务上限 90 分钟；发布任务上限 360 分钟，因为托管 runner 在海外，完整音频和谱面要跨境 PUT 后再 GET 校验。并缓存 `.cache/http` 供后续运行复用；该缓存不进入发布资源包。网络重试和并发均有上限。
