...
首次导入需要读取完整上游资源，耗时受官方服务器网络影响；首次 Ubuntu 实测约 44 分钟。构建与发布任务上限均为 90 分钟。实际上传使用 `--delta-only`：必须能对照线上 catalog/manifest，未变文件 CopyObject，只 PUT 新增和变更；对照不上时拒绝全量跨境 PUT（完整音频首次上线在本地发布）。并缓存 `.cache/http` 供后续运行复用；该缓存不进入发布资源包。网络重试和并发均有上限。
